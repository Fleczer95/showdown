...
Mirrors src/game/progression/achievements.ts (8 tiered families x 3 + 10 one-offs)
...
FAMILIES = [
    ...
    ("challenger", "Challenger", "Rywal", "Play {n} challenges", "Rozegraj {n} wyzwań", [1, 10, 30]),
]

# (family, tier) -> descriptions that the plural {n} template can't express.
DESC_OVERRIDES = {
    ("challenger", "bronze"): ("Play your first challenge", "Rozegraj pierwsze wyzwanie"),
}
...
def achievements():
    """All 34 achievements, in display order."""
    out = []
    for family, en_name, pl_name, en_desc, pl_desc, thresholds in FAMILIES:
        for (tier, points), n in zip(TIERS, thresholds):
            ...
            override = DESC_OVERRIDES.get((family, tier))
            en_text = override[0] if override else en_desc.format(n=_en_num(n))
            pl_text = override[1] if override else pl_desc.format(n=_pl_num(n))
            out.append(
                {
                    "id": local_id,
