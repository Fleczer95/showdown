...
describe('ChallengeHandoff', () => {
    it('reports the full result exactly once, including the progression run', () => {
        ...
        const { rerender } = render(<ChallengeHandoff progress={6} run={run} onComplete={onComplete} />);
        rerender(<ChallengeHandoff progress={6} run={run} onComplete={onComplete} />);
        expect(onComplete).toHaveBeenCalledTimes(1);
        expect(onComplete).toHaveBeenCalledWith({ progress: 6, run });
    });
});
