...
// often, we order candidates so the least-shown come first: a game cycles through
// its whole pool before repeating any, and once every item is equally seen the
// pool reshuffles. Count-based, so it self-cycles with no explicit reset. See
 ...
 * take the first N for a run — guaranteeing the whole pool is cycled before any
 * repeat, while a just-seen item (higher count) sinks to the back.
 */
export function createDeck<T extends { id: string }>(
    items: T[],
    history: History,
    rng: Rng = Math.random,
): T[] {
